    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (result != null) {
            ...
            if (result.getContents() == null) {
                ...
                Intent callIntent = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:" + telp));
                startActivity(callIntent);
            } else if (Patterns.EMAIL_ADDRESS.matcher(result.getContents()).matches()) {
                Intent intent = new Intent(Intent.ACTION_SENDTO, Uri.parse(result.getContents()));
                intent.setType("text/plain");
                intent.putExtra(Intent.EXTRA_EMAIL, new String[]{"esterfiorinpanjaitan@gmail.com"});
                intent.putExtra(Intent.EXTRA_SUBJECT, "Fungsi Email");
                intent.putExtra(Intent.EXTRA_TEXT, "Ester Fiorina Panjaitan");
                try {
